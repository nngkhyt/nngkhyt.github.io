...
#ディレクトリツリーを記述する
def describe_directory_tree(directory):
    tree_lines = []
...
def save_file_contents_with_tree_and_indent(directory,output_filename,max_files):
    file_count = 0
    with open(output_filename, 'w', encoding = 'utf-8')as output_file:        
        #'[ファイル構成]'とディレクトリ構造を出力
        output_file.write("[ファイル構成]\n\n")
        tree = describe_directory_tree(directory)
        ...
        if file_count >= max_files:
            output_file.write("\n[出力上限に達しました]\n")
#ファイル内容の保存を実行
save_file_contents_with_tree_and_indent(current_directory, output_filename, MAX_FILE)