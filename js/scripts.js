...
function filterNews() {
  ...
  newsItems.forEach(function (item) {
    if (
      filterNewsByTag(item) &&
      searchNews(item, input) &&
      filterNewsByDate(item, year, month)
    ) {
...
function populateIngredientDropdown() {
  ...
  const sortedIngredients = Array.from(ingredientSet).sort((a, b) =>
...
function filterRecipes() {
  const selectedIngredients = Array.from(
    document.querySelectorAll(
      '#ingredientDropdownMenu .form-check-input:checked'
    )
  ...
  recipeItems.forEach((item) => {
    const itemIngredients = item
      ...
      .toLowerCase()
      .split(' ');
    const matches = selectedIngredients.map((ingredient) =>
...
function resetRecipeFilters() {
  const checkboxes = document.querySelectorAll(
    '#ingredientDropdownMenu .form-check-input'
  );
...
document.addEventListener('click', function () {
  ...
  if (dropdownMenu.classList.contains('show')) {
    ...
    document
      .getElementById('ingredientDropdownButton')
